...
The adapter emit, by default, the event named `err` for errors, this ensure the data flow (non-blocking state) even if an error occur. For blocking state (no data flow), you can name it `error` on constructor options, or later `adapter.error='error'`.
